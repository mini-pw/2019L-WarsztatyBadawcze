...
correct_duplicated_rows <- function(df){

  powtorki<-distinct(df[duplicated(df[-1]),-1])
  getmode <- function(v) {
    uniqv <- unique(v)
    uniqv[which.max(tabulate(match(v, uniqv)))]
  }
  
  powtorki<-as_tibble(powtorki)
  apply(powtorki,1, function(y){
    temp<-which(apply(df[-1], 1, function(x){all(x==y)}))
    df[temp,1]<<-getmode(df[[1]][temp])
  })
